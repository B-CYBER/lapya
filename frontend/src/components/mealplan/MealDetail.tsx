...
export const MealDetail = ({
  ...
  const ingredients: string[] = ingredientsProp ?? [
  ...
  const cookingSteps: string[] = cookingStepsProp ?? [
    ...
    <div className="w-full h-screen flex flex-col" style={{ backgroundColor: '#FBFAF7' }}>
      ...
      >
        <div className="flex gap-3">
          ...
          <button
            onClick={() => onMarkEaten?.()}
            disabled={isMarking || !onMarkEaten}
            className="flex-1 py-4 rounded-xl transition-all"
            style={{
              ...
              fontWeight: 500,
              border: 'none',
              cursor: isMarking || !onMarkEaten ? 'wait' : 'pointer',
              opacity: isMarking ? 0.6 : 1,
              boxShadow: '0 4px 16px rgba(61, 107, 229, 0.2)'
            }}
          >
            {isMarking ? 'Logging…' : 'I ate this'}
          </button>
        </div>
      </div>
    </div>
